...
- 🔗 **共有リンク**: タイマーごとに専用URLを発行し、同じURLの参加者だけを同期
...
1. `http://localhost:8000/` を開くと、新しいタイマー用のURLが自動作成されます。
2. 画面上部の「リンクをコピー」で、そのURLを参加者へ共有します。
3. 同じ `/room/<部屋ID>` URLを開いた参加者だけが、そのタイマーを操作・閲覧できます。
4. トップページを再度開くと、別の独立したタイマーが作成されます。

部屋とタイマー状態はアプリケーションのメモリ上で管理しています。停止済みかつ未接続で1時間以上使われていない部屋は自動削除され、削除済みURLは404になります。サーバー再起動後も部屋を保持する必要がある場合や、複数コンテナへ拡張する場合は、SQLiteまたはRedisなどの永続ストレージが必要です。
